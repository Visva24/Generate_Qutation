...
export class DeliveryChallanService {
    ...
    async downloadDeliveryChallanTemplate(user_id:number,id: number): Promise<any> {
        try {
            ...
            let  delivery_items = deliveryChalanData.data.delivery_items
            let itemsLength = (delivery_items).length
              let cycleCount = Math.ceil(itemsLength/25 )
                    ...
                    let processQuotationItems = async (id,batchNumber,batchSize,isLastRecord)=>{
                        ...
                        console.log(offset,batchSize);
                         ...
                         let itemData = await this.DeliveryItemModel.findAll({where:{delivery_id:id},
                            limit:batchSize,
                            offset:offset
                        })
                            ...
                            if(incrementedCycle == cycleCount){  
                                ...
                                let startCount = 0
                                   ...
                                   if(incrementedCycle == 1){
                                    ...
                                    let arrayData = await processQuotationItems(deliveryChalanData.data.id,incrementedCycle,startCount,false);
                                    overAllObject[`cycleCount_${incrementedCycle}`].push(...arrayData)
                                    let arrayData1 = await processQuotationItems(deliveryChalanData.data.id,incrementedCycle+1,startCount,false);
